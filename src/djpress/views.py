...
def archive_posts(
    ...
    filtered_posts = Post.post_objects.get_published_posts().filter(published_at__year=date_parts["year"])
    if "month" in date_parts:
        filtered_posts = filtered_posts.filter(published_at__month=date_parts["month"])
    if "day" in date_parts:
        filtered_posts = filtered_posts.filter(published_at__day=date_parts["day"])
